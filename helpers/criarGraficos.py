...
def grafico_linhas_liquidez_trimestral(df):
    ...
    fig.update_traces(textposition='top center')
    st.plotly_chart(fig, use_container_width=True)
# helpers/criarGraficos.py

def grafico_bp(df, ano_inicio, ano_fim, intervalo):
    if intervalo == "Trimestral":
        grafico_bp_trimestral(df, ano_inicio, ano_fim)
    elif intervalo == "Anual":
        grafico_bp_anual(df, ano_inicio, ano_fim)
